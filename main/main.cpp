...
void receiver(void *pvParameters)
{
  ...
  recv.config(CHANNEL, PAYLOAD_SIZE, address, 5, 0, 0, 3);
  recv.printDetails();
  ...
  while (true)
  {
    ...
    if(dec.NewPacket(&cmd, &arg)){
      ESP_LOGI(TAG, "CMD %0d ARG/SEQ %3d", cmd, arg);
    }
    vTaskDelay(50/portTICK_RATE_MS);//Delay ganz am anfang der Schleife, weil es sonst durch die "continue-Aufrufe ggf. nicht Aufgerufen wird"
  }
}
